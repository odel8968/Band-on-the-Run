/* ESP32 MPU9250 Heading Handler
Modified by: Ben Odell
17 October 2020
...
void magcalMPU9250(float * dest1, float * dest2, float fixedMagBias[], float fixedMagScale[], int fixedCalibration) 
{
  ...
  if(fixedCalibration != 1)
  {
    ...
    delay(200);
     ...
     for(ii = 0; ii < sample_count; ii++) {
      ...
      if(Mmode == 0x06) vTaskDelay(12);  // at 100 Hz ODR, new mag data is available every 10 ms
      }
  ...
  else
  {
    ...
    dest1[0] = fixedMagBias[0];
    dest1[1] = fixedMagBias[1];
    dest1[2] = fixedMagBias[2];

    dest2[0] = fixedMagScale[0];
    dest2[1] = fixedMagScale[1];
    dest2[2] = fixedMagScale[2];

    Serial.println("Using hardcoded magnetometer calibration!");
  }
...
void MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
{
    ...
    //Serial.printf("new quaternions: %f %f %f %f \n", q[0], q[1], q[2], q[3]);
}


/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ New Stuff ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
int initHeadingReading(float fixedMagBias[], float fixedMagScale[], int fixedCalibration) //Requires Wire to be initialized already
{
  float buffer1[10] = {}, buffer2[10] = {};
  ...
  accelgyrocalMPU9250(gyroBias, accelBias); //CAUSING PROBLEMS
  ...
  magcalMPU9250(magBias, magScale, fixedMagBias, fixedMagScale, fixedCalibration);
...
// Reads the IMU and Calculates the new Yaw/Pitch/Roll of the vehicle
int updateHeading(float headings[3])
{
  int16_t MPU9250Data[7];

  if (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01) //read int flag for data ready
  {
    uint8_t c = readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);
    byte d = readByte(AK8963_ADDRESS, WHO_AM_I_AK8963);

    if( (c != 0x71) || (d != 0x48) ) //if no connection
      return 0;
    ...
    for(uint8_t i = 0; i < 10; i++) // iterate 10 times per data read
    { 
      Now = micros();
      
      deltat = ((Now - lastUpdate)/1000000.0f); // set integration time by time elapsed since last filter update
      
      //Serial.printf("Last Quaternion calculation: %d microseconds\n", Now - lastUpdate);
      lastUpdate = Now;
  ...
  pitch *= 180.0f / pi;
  yaw   *= 180.0f / pi; 
  yaw   += 6.08f; // Declination at Moscow ID is -6.08 degrees 17 October 2020
